<?php
    ...
    $query = "SELECT `lot_name`, `description`, `image`, `exp_date`, `bet_range`, `category_name` FROM `lots` `l` JOIN `categories` `c` ON `category_id` = `c`.`id` WHERE `l`.`id` = {$_GET['id']}";
    $query = mysqli_query($con, $query);
    $lot = mysqli_fetch_all($query, MYSQLI_ASSOC)[0];
    if (!isset($lot)) {
        header("Location: pages/404.html");
    }
    ...
    $query = "SELECT IF (EXISTS(SELECT 1 FROM `bets` WHERE `lot_id` = {$_GET['id']}), (SELECT MAX(`price`) FROM `bets` WHERE `lot_id` = {$_GET['id']}), (SELECT `start_price` FROM `lots` WHERE `id` = {$_GET['id']})) AS `price`";
    $query = mysqli_query($con, $query);
    $current_price = mysqli_fetch_assoc($query)['price'];

    list($hours, $minutes) = date_range($lot['exp_date']);
?>
...
<div class="page-wrapper">
    ...
    <main>
        <?=include_template('lot_main.php', ['lot' => $lot, 'hours' => $hours, 'minutes' => $minutes, 'categories' => $categories, 'current_price' => $current_price]); ?>
    </main>
